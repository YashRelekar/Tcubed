...
Pi 5 / XVF3800 notes
--------------------
* The reSpeaker XVF3800 (USB ID 2886:001a) records at 48 kHz.  We capture
  blocks of 3840 samples (80 ms at 48 kHz), decimate 3:1 to 1280 samples
  at 16 kHz, and feed that to openWakeWord.
* The mic device name substring is taken from the MIC_NAME env var
  (default "XVF3800") so no code change is required to switch hardware.
"""
...
# Default mic name — overridden by MIC_NAME env var.
_DEFAULT_MIC_NAME = os.getenv("MIC_NAME", "XVF3800")


def _find_mic_device(mic_name: str) -> int:
    """Find the mic device index by name substring."""
...
class WakeWordDetector:
    ...
    def __init__(
        ...
        self.mic_sample_rate = 48000  # XVF3800 native rate
        self.gain_target_peak = gain_target_peak
        # 3840 samples @ 48 kHz = 1280 samples @ 16 kHz (80 ms chunks)
        ...
        print("    Wake word mic: device {} (matched '{}')".format(
            self.mic_device, mic_name))

        # Use custom model if provided and exists; otherwise fall back to hey_jarvis
